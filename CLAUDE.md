...
- **Slice selectors are hooks.** `useModule`, `useNotes`, `useSynthGain`, etc. (exported from `src/reducers/*`) call `useAppSelector` internally — Rules of Hooks apply (top level of components/hooks only).
